...
- **放弃追击**：玩家进入隐藏（`isHidden`）；追到目标点（3D 距离 <1）时玩家已不在视野；
  追击路径无法推进的原地待机分两档——看得见玩家 `CHASE_STUCK_TIMEOUT_MS = 10 s`
  （玩家站箱顶的守箱对峙，放弃后进 6 s 巡逻冷却防 canSee 立刻拉回），看不见玩家
  `CHASE_STUCK_LOST_TIMEOUT_MS = 5 s`（玩家从头顶跳过落到背后离开——残留目标带
  跳跃高度，"到达放弃"永不触发；放弃即回巡逻、无冷却）
...
- **编辑器 /level**：进门口令门禁（POST /api/levels 预校验，localStorage 记住口令，
  ...
  HSV 为唯一事实源，点「确定」才写入草稿）；主题图片上传即前端压缩（等比 ≤512px、
  WebP 优先 JPEG 兜底，单图约 20~100KB；服务端 720K 字符上限仅作兜底）；「保存到云端」
  有整包体积闸（>8MB 弹确认、>24MB 拦截——KV 单值上限 25MiB）；主题数据内嵌于各关
